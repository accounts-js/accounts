...
describe('context-builder', () => {
  ...
  const createOperationControllerMock = jest.fn(() => controllerMock);
  ...
  it('should not resume session is header is empty', async () => {
    ...
    await context(
      { req: { headers: {} } as any },
      { createOperationController: createOperationControllerMock as any }
    );
  ...
  it('should allow no request in context', async () => {
    const data = await context({} as any, {
      createOperationController: createOperationControllerMock as any,
    });
    expect(data.ip).toBe('');
