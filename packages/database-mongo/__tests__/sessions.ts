...
describe('sessions', () => {
  ...
  describe('findSessionById', () => {
    it('should return null for not found session', async () => {
      ...
      const ret = await databaseTests.database.sessions.findSessionById('589871d1c9393d445745a57c');
      expect(ret).not.toBeTruthy();
    });

    it('should find session', async () => {
      const sessionId = await databaseTests.database.sessions.createSession(
        session.userId,
        'token'
      );
  ...
  describe('findSessionByToken', () => {
    it('should return null for not found session', async () => {
      const ret = await databaseTests.database.sessions.findSessionByToken(
    ...
    it('should find session', async () => {
      ...
      await databaseTests.database.sessions.createSession(session.userId, token, {
        ip: session.ip,
        userAgent: session.userAgent,
      });
      const ret = await databaseTests.database.sessions.findSessionByToken(token);
      expect(ret).toBeTruthy();
    });
  });
  ...
  describe('updateSession', () => {
    ...
    it('should update session', async () => {
      ...
      await databaseTests.database.sessions.updateSession(sessionId, {
        ip: 'new ip',
    ...
    it('should update session with token', async () => {
      ...
      await databaseTests.database.sessions.updateSession(
        sessionId,
        {
          ip: 'new ip',
          userAgent: 'new user agent',
  ...
  describe('invalidateAllSessions', () => {
    it('invalidates all sessions', async () => {
      ...
      const sessionId2 = await databaseTests.database.sessions.createSession(
        session.userId,
        generateRandomToken(),
        {
          ip: session.ip,
      ...
      await databaseTests.database.sessions.invalidateAllSessions(session.userId);
      const session1 = await databaseTests.database.sessions.findSessionById(sessionId1);
      const session2 = await databaseTests.database.sessions.findSessionById(sessionId2);
