...
describe('services/password', () => {
  ...
  describe('findUserByResetPasswordToken', () => {
    it('should return null for not found user', async () => {
      const ret = await databaseTests.database.password.findUserByResetPasswordToken('token');
      expect(ret).not.toBeTruthy();
    });

    it('should return user', async () => {
      const userId = await databaseTests.database.createUser(user);
      await databaseTests.database.password.addResetPasswordToken(
        userId,
        'john@doe.com',
        'token',
        'test'
  ...
  describe('findUserByEmailVerificationToken', () => {
    it('should return null for not found user', async () => {
      const ret = await databaseTests.database.password.findUserByEmailVerificationToken('token');
      expect(ret).not.toBeTruthy();
    });

    it('should return user', async () => {
      const userId = await databaseTests.database.createUser(user);
      await databaseTests.database.password.addEmailVerificationToken(
        userId,
        'john@doe.com',
        'token'
      );
  ...
  describe('findPasswordHash', () => {
    ...
    it('should return null on not found user', async () => {
      const ret = await databaseTests.database.password.findPasswordHash(
    ...
    it('should return hash', async () => {
      ...
      const ret = await databaseTests.database.password.findPasswordHash(userId);
  ...
  describe('setPassword', () => {
    ...
    it('should throw if user is not found', async () => {
      await expect(
        databaseTests.database.password.setPassword('589871d1c9393d445745a57c', 'toto')
    ...
    it('should change password', async () => {
      ...
      const userId = await databaseTests.database.createUser(user);
      await databaseTests.database.password.setPassword(userId, newPassword);
      const retUser = await databaseTests.database.findUserById(userId);
      const services: any = retUser!.services;
  ...
  describe('setResetPassword', () => {
    it('should change password', async () => {
      const newPassword = 'newpass';
      const userId = await databaseTests.database.createUser(user);
      await databaseTests.database.password.setResetPassword(userId, 'toto', newPassword);
      const retUser = await databaseTests.database.findUserById(userId);
      const services: any = retUser!.services;
      expect(services.password.bcrypt).toBeTruthy();
      expect(services.password.bcrypt).toEqual(newPassword);
  ...
  describe('addEmail', () => {
    ...
    it('should throw if user is not found', async () => {
      await expect(
        databaseTests.database.password.addEmail('589871d1c9393d445745a57c', 'unknowemail', false)
      ).rejects.toThrowError('User not found');
    });
    ...
    it('should add email', async () => {
      ...
      const userId = await databaseTests.database.createUser(user);
      await databaseTests.database.password.addEmail(userId, email, false);
  ...
  describe('removeEmail', () => {
    ...
    it('should throw if user is not found', async () => {
      await expect(
        databaseTests.database.password.removeEmail('589871d1c9393d445745a57c', 'unknowemail')
      ).rejects.toThrowError('User not found');
    });
    ...
    it('should remove email', async () => {
      ...
      const userId = await databaseTests.database.createUser(user);
      await databaseTests.database.password.addEmail(userId, email, false);
      await databaseTests.database.password.removeEmail(userId, user.email);
      const retUser = await databaseTests.database.findUserById(userId);
    ...
    it('should remove uppercase email', async () => {
      ...
      const userId = await databaseTests.database.createUser(user);
      await databaseTests.database.password.addEmail(userId, email, false);
      await databaseTests.database.password.removeEmail(userId, 'JOHN@doe.com');
      const retUser = await databaseTests.database.findUserById(userId);
      expect(retUser!.emails!.length).toEqual(1);
      expect(retUser!.emails![0].address).toEqual(email);
    });
  ...
  describe('verifyEmail', () => {
    ...
    it('should throw if user is not found', async () => {
      await expect(
        databaseTests.database.password.verifyEmail('589871d1c9393d445745a57c', 'unknowemail')
      ).rejects.toThrowError('User not found');
    });
    ...
    it('should verify email', async () => {
      ...
      await databaseTests.database.password.verifyEmail(userId, user.email);
